...
def subsampled_dset(shapes, n_trials):
	...
	while seq.shape[0] < n_trials:
		...
		comb = shapes[:4]
		same_targ = np.round(np.random.rand()).astype(int)
		if same_targ == 0:
		...
		np.random.shuffle(shapes)
		comb = shapes[:5]
		diff_targ = np.round(np.random.rand()).astype(int)
		if diff_targ == 0:
...
def full_dset(shapes, n_trials):
	...
	if len(all_same_seq) < n_trials/2:
		...
		for a in range(int(n_trials/2) - len(all_same_seq)):
			trial_ind = np.floor(np.random.rand() * len(all_same_seq)).astype(int)
	...
	if len(all_diff_seq) < n_trials/2:
		...
		for a in range(int(n_trials/2) - len(all_diff_seq)):
			trial_ind = np.floor(np.random.rand() * len(all_diff_seq)).astype(int)
...
def create_task(args, train_shapes, test_shapes):
	...
	if args.m_holdout == 0:
		...
		if args.train_set_size + args.test_set_size > total_trials:
			...
			args.train_set_size = np.round(train_proportion * total_trials).astype(int)
			log.info('Changing training set size to ' + str(args.train_set_size) + '...')
			args.test_set_size = np.round(test_proportion * total_trials).astype(int)
